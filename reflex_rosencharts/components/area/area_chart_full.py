"""area_chart_full — port of rosencharts area-charts/2_AreaChartFull.tsx to Reflex.

Render technique: D3 ``scaleTime``/``scaleLinear`` with ``d3.area`` + ``d3.line``
(curveMonotoneX), drawn edge-to-edge (no margins). Uses the shared ClientTooltip
helper (createPortal), so the component renders client-side only (NoSSRComponent).
...
Example::
    ...
    rxc.area_chart_full(data=State.sales, color="text-blue-200 dark:text-blue-400")
"""
...
class AreaChartFull(rx.NoSSRComponent):
    """Reflex wrapper around the parametrized AreaChartFull TSX."""
    ...
    color: rx.Var[str] = rx.Var.create("text-blue-200 dark:text-blue-400")


def area_chart_full(**props) -> rx.Component:
    """Edge-to-edge filled area chart over a time series, with a per-point tooltip.
    ...
    Args:
        ...
        color: Tailwind ``text-*`` class for the filled area. Defaults to
            ``"text-blue-200 dark:text-blue-400"``.

    Returns:
        A Reflex component rendering the full-bleed area chart.
    """
