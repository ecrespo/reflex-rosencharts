"""area_chart_semi_filled — port of rosencharts area-charts/4_AreaChartSemiFilled.tsx.

Render technique: D3 ``d3.area`` + ``d3.line`` (curveMonotoneX). The area uses a
faint SVG ``<linearGradient>`` (semi-transparent stops) under a solid stroked line.
Uses the shared ClientTooltip helper (createPortal), so it renders client-side only
(NoSSRComponent).
...
class AreaChartSemiFilled(rx.NoSSRComponent):
    """Reflex wrapper around the parametrized AreaChartSemiFilled TSX."""
    ...
    color: rx.Var[str] = rx.Var.create("text-yellow-400 dark:text-yellow-600")
    color_from: rx.Var[str] = rx.Var.create("text-yellow-500/20 dark:text-yellow-500/20")
    color_to: rx.Var[str] = rx.Var.create("text-yellow-50/0 dark:text-yellow-900/0")


def area_chart_semi_filled(**props) -> rx.Component:
    """Semi-filled (faint gradient) area chart over a time series, with tooltip.

    Args:
        data: list of ``{"date": "YYYY-MM-DD", "value": float}``. Defaults to the
            original rosencharts example.
        color: Tailwind ``text-*`` class for the stroked line.
        color_from: Tailwind ``text-*`` class for the top gradient stop.
        color_to: Tailwind ``text-*`` class for the bottom gradient stop.

    Returns:
        A Reflex component rendering the semi-filled area chart.
    """
