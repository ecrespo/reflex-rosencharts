...
/* Ported from rosencharts (MIT, Filsommer): area-charts/4_AreaChartSemiFilled.tsx. Data is now a prop. */

type AreaPoint = { date: string; value: number };

const defaultData: AreaPoint[] = [
...
export function AreaChartSemiFilled({
  ...
  colorFrom = "text-yellow-500/20 dark:text-yellow-500/20",
  colorTo = "text-yellow-50/0 dark:text-yellow-900/0",
}: {
  data?: AreaPoint[];
  color?: string;
  colorFrom?: string;
  ...
  let line = d3line<(typeof parsed)[number]>()
    .x((d) => xScale(d.date))
    .y((d) => yScale(d.value))
    .curve(curveMonotoneX);

  // Area generator
  let area = d3area<(typeof parsed)[number]>()
    .x((d) => xScale(d.date))
    .y0(yScale(0))
    .y1((d) => yScale(d.value))
    .curve(curveMonotoneX);

  let areaPath = area(parsed) ?? undefined;
    ...
    <div className="relative h-72 w-full">
      ...
      >
        ...
        >
          ...
          <defs>
            ...
            <linearGradient id="outlinedAreaGradient" x1="0" x2="0" y1="0" y2="1">
              <stop offset="0%" className={colorFrom} stopColor="currentColor" />
              <stop offset="90%" className={colorTo} stopColor="currentColor" />
            </linearGradient>
          </defs>
        ...
        {parsed.map((day, i) => {
          ...
          if (i % 6 !== 0 || i === 0 || i >= parsed.length - 3) return;
          return (
            <div
              key={i}
              style={{
